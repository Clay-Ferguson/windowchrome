...
## What this is
This is a library that takes care of application window and dialog styling so that we can share it across all our Python projects. Several unrelated things live here: the colored title bar, a markdown viewer for showing an app's own `docs/` inside itself, the wide scroll bars the apps use everywhere, and three widget looks the apps share — the enlarged radio indicator, the enlarged check-box indicator and the on/off toggle switch.

## Modules

- `theme.py` — `ChromeTheme`, the colors, the title font's weight and stretch and the Wayland decoration plugin name, plus the module-level active theme. Two of the defaults are decisions rather than neutral values, and both are commented in place: `title_fg_inactive` matches `title_fg` so the title does not dim while the window is being dragged, and there is no point-size field because the plugin compiles one in.
- `titlebar.py` — `configure()` / `install()`, the body-color accessors and `body_font()`. Order-sensitive and Wayland-only; README §4 and §6 are the contract. The font half (`_install_title_font`) is the same trade as the palette half — the decoration has no font of its own, so the title's weight has to go on the *application* font and the body is handed its font back — but it is handed back as a class font rather than through the polish filter, and the two `setFont` calls inside it are not reorderable. README gotcha 5 and the module docstring both say why; `tests/test_titlefont.py` is the guard, and is the only part of the title bar testable off Wayland.
...
**A `QPainter` over a pixmap now starts with the *title* font, and that is the mechanism rather than a bug.** It is the exact path `bradient` takes to draw the title — a painter on a non-widget device resolves `QGuiApplication::font()` — which is why the title's weight has to live on the application font at all. Widgets are unaffected (the class font hands theirs back), so the only code that has to care is code drawing text on a pixmap, and `body_font()` is what it reaches for. `toggleswitch.py` paints on `self` and draws no text, so nothing in this library needs it; the host apps' drawn fallback icons do.

**`scrollbars.py` and `radiobuttons.py` read `QPalette.Base` (and `Text`) straight from the application palette, and that is not the gotcha-3 mistake.** Gotcha 3 forbids deriving a *body* color from `QApplication.palette()` because `install()` repurposes `Window` and `WindowText` for the title bar. Neither `Base` nor `Text` is one of them, so reading them here is correct and routing it through `body_window_color()` would be wrong. Don't "fix" it. `checkboxes.py` reads no palette color at all — the native drawing it keeps already follows the theme, and only the size was ever wrong. `toggleswitch.py` reads neither: its off track and knob are pinned grays, because `Base` is the pane the switch sits *on* and a switch painted in it would vanish — that is a decision, not an oversight (README §11).
